...
class ElasticsearchStatusCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $indexTask = $this->connection->fetchAssociative('SELECT * FROM elasticsearch_index_task WHERE entity = "blog" LIMIT 1');
        $totalBlogs = (int) $this->connection->fetchOne('SELECT COUNT(*) FROM blog WHERE version_id = :liveVersionId AND child_count = 0 OR parent_id IS NOT NULL', ['liveVersionId' => Uuid::fromHexToBytes(Defaults::LIVE_VERSION)]);
        ...
        if ((int) $indexTask['doc_count'] > 0) {
            ...
            $progressBar = new ProgressBar($output, $totalBlogs);
            $progressBar->advance($totalBlogs - $indexTask['doc_count']);
            $output->writeln('');
        } else {
            $table->addRow(['Indexing', 'completed']);
            $table->render();
